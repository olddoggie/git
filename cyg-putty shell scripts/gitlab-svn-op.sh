...
project=op
svnURL="file:///home/svn/repos/openproperty/trunk/OP" # used by svn diff --summerize in order to fetch the deleted files since last tag
...
svn export --force -r HEAD $svn_repo $git_repo/OP # attention, svn export to subfolder OP
svn diff -r $PRE:HEAD --summarize "$svnURL" | sed 's#%20# #g' | grep "\." | sed 's#'"$svnURL"'/##g' | grep "D       " | sed 's#D       #OP/#g' | sort -u | xargs -i git rm -rf --ignore-unmatch {}
