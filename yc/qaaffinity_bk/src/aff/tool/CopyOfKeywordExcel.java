 ...
 * 
 * @author jyu
 * This class export the parameter of the keyword(method) in the help column
 */
public class CopyOfKeywordExcel {
		...
		for (Method m : mArr) {
			if (Modifier.isPublic(m.getModifiers())) {
				...
				if (!methodsList.isEmpty()) {
					keywordList.add(methodsList.toString());
				}
	...
	public static void writeExcel(String filePath, List<List<String>> keywordList, List<List<String>> libraryList) throws BiffException, IOException, WriteException {

		//The default file only has the title column for keywords.
		//The default file has not keywords
		Workbook defaultWorkbook = Workbook.getWorkbook(new File("resource/xls/Library.xls"));
	...
	public static void main(String[] args) throws ClassNotFoundException, BiffException, IOException, WriteException {
		...
		//writeExcel("resource/xls/keyword_template.xls", keywordList, libraryList);
		writeExcel("resource/xls/aff_keyWord_library.xls", keywordList, libraryList);
